...
CHAIN_DIR=chains/exotic_perturbed/chain_${PROCESS}
LOGFILE=logs/exotic_perturbed_chain_${PROCESS}.out
LOCK_FILE="$CHAIN_DIR/exotic_perturbed.input.yaml.locked"

mkdir -p "$CHAIN_DIR" logs

PER_CHAIN_YAML=$CHAIN_DIR/exotic_perturbed_${PROCESS}.yaml
sed "s|^output:.*|output: ${CHAIN_DIR}/exotic_perturbed_${PROCESS}|" \
