...
int background_functions(
  ...
  // Scalings - Luis Rufino (8-19-25)
  double n = pba->n_e;
  double m = pba->m_s;
  // Densities - Luis Rufino
  double rho_s_star; // Standard fixed point
  double rho_e_star; // exotic fixed point 
  rho_e_star = 3.0 * (m - 2.0)/(m - n) * pow(pba->a_star, n - 2.0);
  rho_s_star = 3.0 * (2.0 - n)/(m - n) * pow(pba->a_star, m - 2.0);
  double rho_e = 0.0;
  double rho_s = 0.0;
  ...
  rho_r=0.;
  rho_m=0.;
  //printf("Hello background_functions\n");
  class_test(a <= 0.,
             pba->error_message,
  ...
  {
    /* Compute fixeed point densities */
    double H_conformal = pvecback_B[pba->index_bi_a_prime] / a;
    double H_prop = H_conformal / a;

    double a_prime = pvecback_B[pba->index_bi_a_prime];
    //double a_dprime = dy[pba->index_bi_a_prime]; // careful: only available in derivs
    //double H_prime_conformal = (a_dprime/a) - H_conformal*H_conformal;
    //double H_prime_prop = (H_prime_conformal - H_conformal*H_conformal)/ (a*a);
    ...
    rho_tot = pvecback[pba->index_bg_rho_e] + pvecback[pba->index_bg_rho_s];
  ...
  else
  {
      ...
      pvecback[pba->index_bg_a] = a;
      //  printf("[BACKGROUND_FUNCTIONS] a = %.5e\n", a); // This needs to be same value from post to standard. 

      /** - compute each component's density and pressure */

      if (pba->has_exotic == _TRUE_)
      {
        pvecback[pba->index_bg_rho_e] = pba->Omega0_e * pow(pba->H0,2) / pow(a,n);
        rho_tot += pvecback[pba->index_bg_rho_e];
        rho_e   += pvecback[pba->index_bg_rho_e];
      }

      /* photons */
      pvecback[pba->index_bg_rho_g] = pba->Omega0_g * pow(pba->H0,2) / pow(a,4);
      //printf("rho_g = %.5e\n",pba->Omega0_g * pow(pba->H0,2) / pow(a,4));
...
int background_indices(
  ...
  /* Adding Delta and Delta_prime (Luis Rufino, 7-23-25)*/
  printf("[BACKGROUND_INDEX][DEBUG] bg_regime = %d\n", pba->bg_regime);

  if(pba->bg_regime > 0)
  {
    printf("[BACKGROUND_INDEX] [DEBUG] ENTERED LINGERING UNIVERSE\n");
    class_define_index(pba->index_bi_Delta, _TRUE_, index_bi, 1);
    class_define_index(pba->index_bi_Delta_prime, _TRUE_, index_bi, 1);
    class_define_index(pba->index_bi_a, _TRUE_, index_bi, 1);
    class_define_index(pba->index_bi_a_prime, _TRUE_, index_bi, 1);
    class_define_index(pba->index_bi_tau_trans, _TRUE_, index_bi, 1);
    printf("[BACKGROUND_INDEX] index_bi_Delta = %d\n", pba->index_bi_Delta);
    printf("[BACKGORUND_INDEX] index_bi_Delta_prime = %d\n", pba->index_bi_Delta_prime);
    // Above prints the index number of these variables
    // To print the stored values, do the following
    //printf("Delta = %.5e\n", pvecback_integration[pba->index_bi_Delta]);
    //printf("Delta_prime = %.5e\n", pvecback_integration[pba->index_bi_Delta_prime]); // -> 1.00e-02
    // The printf wont work since pvecback is not declared in this method, it's just an example. 
  }
...
int background_solve(
  ...
  double * y;  // background variables to be integrated
  ...
  //printf("bg_regime: %d\n", pba->bg_regime);
  while (pba->bg_regime != bg_regime_standard)
  {
      if (pba->bg_regime == bg_regime_lingering_exact)
      {
        int status; 
  ...
  printf("[BACKGROUND_SOLVE] loga_first = %.5e  loga_last = %.5e  a_min = %.5e\n",
...
//  }
  ...
  {
    a = pba->a_star;
    //---------- scale factor inital conditions ----------/
    pvecback_integration[pba->index_bi_a] = a;
    pvecback_integration[pba->index_bi_a_prime] = pba->a_star * 0.01;
    //---------- Hubble inital conditions ----------/
    pvecback[pba->index_bg_H] = 0.0;
    pvecback[pba->index_bg_H_prime] = 0.0;
    //---------- Delta inital conditions ----------/
    pvecback_integration[pba->index_bi_Delta] = 0.0; // Setting intial ∆
    pvecback_integration[pba->index_bi_Delta_prime] = 0.0; // Setting ∆_prime to start exist of exact lingering. 
    // (Luis Rufino, 7-23-25)
    //---------- "Clocks"  inital conditions ----------/
    //pvecback_integration[pba->index_bi_rs] = 0.0;   // sound horizon initially zero in the exact phase
    pvecback_integration[pba->index_bi_tau] = 0.0;   // start conformal clock
    ...
    if (pba->background_verbose > 0) {
      ...
      printf("         a_ini = %.1e\n", a);
      printf("[CLASS] [lingering_ICs] Initial H = %.5e\n", pvecback[pba->index_bg_H]);
      printf("[CLASS] [lingering_ICs] Initial H' = %.5e\n", pvecback[pba->index_bg_H_prime]);
    }

    /* -- Populate pvecback (standard CLASS vector of background quantities) -- */
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)rho_e",pba->has_exotic); // Writes to .dat file (Luis Rufino, 6-10-25)
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_e],pba->has_exotic,storeidx);  // Luis Rufino 6-10-25: Writing Exotic fluid to .dat file
...
int background_lingering_derivs(double tau, double* y, 
                                ...
                                ErrorMsg error_message)
{
    ...
    double Delta_e = pba->Delta_e;
    double Delta_s = pba->Delta_s;
    double Delta_rho_s = pba->Delta_rho_s;
    double a_star = pba->a_star;

    double rho_tot_ling;
    rho_tot_ling = 0;// Will be used later in background_functions()

    double tau_values = y[pba->index_bi_tau];

    // Exact fluid values
    double rho_e_star, rho_s_star;
    rho_e_star = 3.0 * (m - 2.0)/(m - n) * pow(pba->a_star, n - 2.0);
    rho_s_star = 3.0 * (2.0 - n)/(m - n) * pow(pba->a_star, m - 2.0);

    //printf("[DEBUG LINGERING] tau = %.5e\n", pvecback[pba->index_bi_tau]);

    dy[pba->index_bi_tau] = 1.0;
    //dy[pba->index_bi_time] = a;   // dt/dτ = a // conformal scale factor

    /*---------------- EXACT LINGERING ----------------*/
    if (pba->bg_regime == bg_regime_lingering_exact)
    {

        double Delta       = y[pba->index_bi_Delta];
        double Delta_prime = y[pba->index_bi_Delta_prime];
        double Delta_dprime; //= dy[pba->index_bi_Delta_prime]; // Better if I call this later

        // === Compute a, a', and a'' ===
        double a = pba->a_star * (1.0 + Delta);
        //dy[pba->index_bi_time] = a;   // dt/dτ = a // conformal scale factor
        // === Evolve Delta and Delta' ===
        dy[pba->index_bi_Delta] = Delta_prime;

        if (n == 2.0)
        {
            // === case: n = 2 ===
            double a_star_pow = pow(pba->a_star, m - 2.0);
            double b_coeff = (1.0/6.0) * (((m - 3.0) * Delta_rho_s/a_star_pow) -  3.0 * Delta_e);
            //printf("n = 2 Case\n\t b_coeff: %.5e\n", b_coeff);
            //dy[pba->index_bi_Delta_prime] = -b_coeff;
            //Delta_dprime = -b_coeff; // This simplifies to Δ'' = 1/2(m-2)\Delta_e
            Delta_dprime = (m - 2.0) * Delta_e * 0.5;
        }
        else
        {
            // === General case: n < 2 ===
            double a_coeff = 0.5 * (m - 2.0) * (n - 2.0);
            double b_coeff = 0.5 * ((m - 2.0) * (n - 3.0) * Delta_e - (m - 3.0) * (n - 2.0) * Delta_s) / (m - n);
            Delta_dprime = -a_coeff * Delta - b_coeff;
            //dy[pba->index_bi_Delta_prime] = -a_coeff * Delta - b_coeff;
        }
        dy[pba->index_bi_Delta_prime] = Delta_dprime;
        // === Compute a', and a'' ===
        double a_prime  = pba->a_star * Delta_prime;
        double a_dprime = pba->a_star * Delta_dprime;
        dy[pba->index_bi_a] = a_prime;   // evolve a
        dy[pba->index_bi_a_prime] = a_dprime;  // evolve a'
        dy[pba->index_bi_time] = a;   // dt/dτ = a // conformal scale factor
        y[pba->index_bi_a] = a; // keep y in sync


        // printf("[DEBUG LINGERING] Delta = %.5e, Delta' = %.5e\n", Delta, Delta_prime);
        // printf("[DEBUG LINGERING] dy[Delta] = %.5e\n", dy[pba->index_bi_Delta]);
        // printf("[DEBUG LINGERING] dy[Delta'] = %.5e\n", dy[pba->index_bi_Delta_prime]);

        // === Compute conformal Hubble ===
        double H_conformal = a_prime/a; // Conformal Hubble
        double H_prop = H_conformal/a; // Proper Hubble
        double H_prime = (a_dprime/a) - H_conformal * H_conformal; // Conformal H'
        double H_prime_prop = (H_prime - pow(H_conformal,2))/pow(a,2);
        // printf("[EXACT LINGERING] H_conf = %.5e H_prop = %.5e\n", H_conformal, H_prop);
        ...
        if(Delta > 1.0)
        {
            printf("--------------[LINGERING->POST]--------------\n");
            printf("Transition triggered\n");
            printf("tau = %.5e Delta = %.5e, H = %.5e, a = %.5e\n", 
                    tau_values, Delta, H_conformal, pba-> a_star*(1.0 + Delta));
            /* Inital conditions for the next phase*/
            // pvecback[pba->index_bg_rho_tot] = rho_tot_ling;

            pba->H_trans = H_conformal;   // Conformal Hubble. These will be used for the transition
            pba->H_prime_trans = H_prime; // Conformal Hubble
            pba->a_trans = a;
            pba->a_exact = pba->a_star;
            pba->a_prime_trans = a_prime;
            pba->tau_trans  = y[pba->index_bi_tau]; // Better notation
            pba->time_trans = y[pba->index_bi_time]; // Better notation
            printf("Scale factors at transitions\n");
            printf("a = %.5e, a' = %.5e\n", pba->a_trans, pba-> a_prime_trans);
    ...
    /*---------------- POST LINGERING ----------------*/
    if (pba->bg_regime == bg_regime_post_lingering)
    {
        //pba->flag_transitioned = _FALSE_;
        ...
        double H_conformal = aprime / a;
        double H_prop = H_conformal / a;
        double Hprime = (adprime/a) - H_conformal * H_conformal;
        double epsilon = 2.0 - (adprime * a/(aprime * aprime));
        double H_prime_prop = (Hprime - pow(H_conformal,2))/pow(a,2);
        dy[pba->index_bi_a] = aprime;
        dy[pba->index_bi_a_prime] = adprime;

        //printf("[DEBUG POST-LINGERING]\nH = %.5e H' = %.5e tau = %.5e a = %.5e a' = %.5e a'' = %.5e\n",
               //H_conformal, Hprime,  tau, a, aprime, adprime);
        //printf("[DEBUG POST-LINGERING]\nepsilon = %.5e\n", epsilon);

        /* Transition to inflation regime if needed */
        // Example: check when a gets large or epsilon < 1
        // printf("H_conf = %.5e H_prop = %.5e\n", H_conformal, H_prop);

        // Computing energy densities, and store them in each pvecback array
        rho_e_trans = rho_e_star * pow(pba->a_trans, -n);
        rho_s_trans = rho_s_star * pow(pba->a_trans, -m);

//        pvecback[pba->index_bg_rho_e] = rho_e_trans * pow(a / pba->a_trans, -n);
//        pvecback[pba->index_bg_rho_s] = rho_s_trans * pow(a / pba->a_trans, -m);
...
//        pvecback[pba->index_bg_a] = a;
        ...
        if (epsilon > 0.0 && epsilon < 0.01)
        {
            ...
            pba->a_trans = a;
            pba->a_post = a;
            pba->a_prime_trans = aprime;
            pba->H_trans = H_conformal;
            pvecback[pba->index_bg_H] = H_conformal / a; // Allocating the proper Hubble
...
int background_derivs(
  ...
  a = exp(loga);
  //printf("[BACKGROUND DERIVS] H_prop = %.5e\n", H);
  //printf("[BACKGROUND DERIVS] H_trans = %.5e\n", pba->H_trans);
  //printf("[BACKGROUND DERIVS] a = %.5e\n", a);
  ...
  //printf("[BACKGROUND DERIVS] AFTER CALLING BACKGROUND_FUNCTIONS  H_prop = %.5e\n", H); 
...
int background_sources(
  ...
  /** - scale factor a (in fact, given our normalisation conventions, this stands for a/a_0) */
//  if (pba->bg_regime == bg_regime_lingering_exact || pba->bg_regime == bg_regime_post_lingering)
//  {
//    a = y[pba->index_bi_a]; // or pba->a_star*(1+Δ) in exact phase
//  }
//  else {
//    a = exp(loga);
//  }
