...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /** 7.2.5) Exotic lingering fluid (Luis Rufino, 6-10-25) */

  /* Attempt to read the exotic density fraction today, n_e exponent, and a_star scale factor
     from the .ini file. If a parameter is not found, its corresponding flag (e.g., flag1) will be _FALSE_ */
  class_call(parser_read_double(pfc, "Omega_e", &param1, &flag1, errmsg),
             errmsg, errmsg);

  class_call(parser_read_double(pfc, "n_e", &param_n_e, &flag_n_e, errmsg),
             errmsg, errmsg);
  class_call(parser_read_int(pfc, "s_map", &param_s_map, &flag_s_map, errmsg),
            errmsg, errmsg);

  class_call(parser_read_double(pfc, "m_s", &param_m_s, &flag_m_s, errmsg),
            errmsg, errmsg);

  class_call(parser_read_double(pfc, "a_star", &param4, &flag4, errmsg),
             errmsg, errmsg);

  class_call(parser_read_double(pfc, "Delta_e", &param_Delta_e, &flag_Delta_e, errmsg), 
             errmsg, errmsg);

  class_call(parser_read_double(pfc, "Delta_s", &param_Delta_s, &flag_Delta_s, errmsg),
             errmsg, errmsg);

  class_call(parser_read_double(pfc, "Delta_rho_s", &param_Delta_rho_s, &flag_Delta_rho_s, errmsg),
            errmsg, errmsg);
  ...
  class_read_flag("has_exact_ling", pba->has_exact_ling);
  ...
  if (flag1 == _TRUE_) {
    ...
    /* Compute the corresponding equation of state: w_e = n_e/3 - 1 */
    pba->w_e = pba-> n_e / 3.0 - 1.0;

    /* Use provided a_star if available, otherwise default to a very early scale factor */
    pba->a_star = (flag4 == _TRUE_) ? param4 : 1e-12;
    ...
    printf("  w_e       = %f\n", pba->w_e);
    printf("  a_star    = %e\n", pba->a_star);
    printf("  exact_lingering = %s\n", (pba->has_exact_ling == _TRUE_) ? "true" : "false");
  }
  else {
    /* If Omega_e wasn't found, treat the exotic fluid as absent */
    pba->Omega0_e = 0.0;
  ...
  Omega_tot += pba->Omega0_e; // <-- Exoctic energy (Luis Rufino, 6-10-25)
