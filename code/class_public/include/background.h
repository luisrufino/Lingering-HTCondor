...
typedef enum
{
    bg_regime_standard,         // 0
    bg_regime_lingering_exact,  // 1
    bg_regime_post_lingering    // 2
} bg_regime;

typedef enum
{
    s_cdm, // 0 s mapping to CDM
    s_b,   // 1 s maping to baryons
    s_ur,  // 2 s mapping to ultra rel
    s_g,    // 3 s mapping to photons
    s_keep_separate // 4
} s_map;

#define LINGER_INIT_CAP  4000 // maxium storage
...
struct background
{
  ...
  //@{
  int bg_regime; // Enum tracking to swtich regimes

  /* Adding new lingering varaibles - Luis Rufino 6-9-25*/
  double Omega0_e;  // The fractional density of rho_e_0/rho_e_crit
  double Omega0_s;  // The standard fluid
  double w_e;       // Equation of state for rho_e
  double w_s;       // Equation of state for rho_s

  double m_s;       // Scaling for rho_s \sim a^-m
  double n_e;       // The scaling for rho_e \sim a^-n

  short has_exotic; // Presence of exotic matter.

  int index_bg_rho_e;  // lingering exotic fluid energy density */
  int index_bg_p_e;    // lingering exotic fluid pressure (optional if w is constant) */
  int index_bg_rho_s; // energy density of the exotic fluid
  int index_bg_p_s;   // pressure of the exotic fluid

  /* === Lingering-to-post-lingering transition values (Luis Rufino 8-6-25) === */
  /* Used for reinitializing y[] after lingering -> post-lingering switch */
  double a_trans;
  double a_exact;
  double a_post;
  double H_trans;
  double H_prime_trans;
  double a_prime_trans;
  double tau_trans;
  double time_trans;
  short flag_transitioned;

  // standard matter mapping
  int s_map;

  double a_star;        // Scale factor for exact lingering

  short has_exact_ling; // Boolean for exact lingering phase

  double Delta_e;        // Exotic fluid deviation
  double Delta_s;        // Standard fluid deviation for n < 2
  double Delta_rho_s;    // Standard fluid deviation for n == 2
  double Delta_eta_0;   // initial scale factor deviation

  /* Done adding new species */
  ...
  /* interpolation tables for the lingering phases (Luis Rufino, 8-20-25)*/
  int    ling_N_alloc;   // capacity
  int    ling_N_used;    // number of rows written
  double *ling_tau;      // conformal time eta
  double *ling_a;        // scale factor a(eta)
  double *ling_H;        // conformal Hubble H(eta) = a'/a
  double *ling_eps;      // slow-roll epsilon(eta) if you have it (or -1 if N/A)

  // simple sampling control to avoid writing every substep
  double ling_min_deta;  // write only if Delta eta >= this (set in precision) 

  // book-keeping: last write 
  double _ling_last_tau;

  /* End of additions Luis Rufino */
  //@}
  ...
  int index_bi_D;       /**< {C} scale independent growth factor D(eta)*/
  ...
  int index_bi_a;
  int index_bi_a_prime;
  int index_bi_Delta;
  int index_bi_Delta_prime;
  int index_bi_tau_trans;
...
extern "C" {
  ...
  /** 
   * Evolve Delta, rho_s, rho_e during lingering phase    */
  int background_lingering_derivs(
  double tau,
  double * y,
  double * dy,
